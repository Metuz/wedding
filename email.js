...
build_html = (data) => {
	...
	if(data.no_guests == '0'){
		...
		str = `<h3>${data.user_name}</h3><p>Su teléfono es ${data.tel}</p><p>Asistira con ${data.no_guests} invitados</p><p>${data.guest_names}</p><p>Les desea lo siguiente</p><p>${data.wish}</p>`
	};
	return str;
}
